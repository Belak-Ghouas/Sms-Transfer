...
class UserRemoteDataSourceImpl(
        ...
        try {
            return if (phoneUtils.isNetworkOnline()) {
                ...
                if (loginResponse.isSuccessful) {
                    ...
                    val errorApi =
                        Gson().fromJson(jsonObject?.get("error").toString(), ErrorApi::class.java)
    ...
    override suspend fun deleteAccount(token: String): Result<Unit> {
        return try {
            if (phoneUtils.isNetworkOnline()) {
                val loginResponse = userApi.deleteAccount("Bearer $token")
                if (loginResponse.isSuccessful) {
                    Result.Success(Unit)
                } else {
                    val responseBodyString = loginResponse.errorBody()?.string()
                    Result.Failure(loginResponse.code(), responseBodyString)
                }
            } else {
                Result.Failure(errorCode = NO_NETWORK, "no network")
            }
        } catch (exception: Exception) {
            Result.Failure(errorCode = INTERNAL_ERROR, "Something goes wrong")
        }
    }
    /*
